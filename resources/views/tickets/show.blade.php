      ...
      <div class="card-body p-4">
        <div class="d-flex justify-content-between align-items-start mb-4">
          <div>
            ...
            <p class="text-muted mb-0 small">Dibuat pada: {{ $ticket->created_at->format('d M Y, H:i') }}</p>
          </div>
          @if(true)
          <div class="dropdown">
            <button class="btn btn-light dropdown-toggle" type="button" data-bs-toggle="dropdown">
              <iconify-icon icon="solar:settings-linear" class="me-2"></iconify-icon>Aksi
            </button>
            <ul class="dropdown-menu">
              ...
              <li>
                <a class="dropdown-item" href="#" onclick="assignTechnician()">
        ...
        <div class="row mb-4">
          ...
          <div class="col-md-6">
            <div class="border rounded-3 p-3 bg-light @if($ticket->sla_due_date && $ticket->sla_due_date < now() && in_array($ticket->status, ['open', 'in_progress'])) border-danger @elseif($ticket->sla_due_date && $ticket->sla_due_date->diffInHours(now()) < 12) border-warning @else border-success @endif">
              <small class="text-muted d-block mb-1">SLA Due</small>
              <span class="fw-semibold @if($ticket->sla_due_date && $ticket->sla_due_date < now() && in_array($ticket->status, ['open', 'in_progress'])) text-danger @elseif($ticket->sla_due_date && $ticket->sla_due_date->diffInHours(now()) < 12) text-warning @else text-success @endif">
          ...
          <div class="row g-2">
            ...
            <div class="col-4">
              <div class="border rounded-3 p-2 text-center">
                <iconify-icon icon="solar:file-linear" class="fs-3 text-primary mb-2"></iconify-icon>
                <p class="mb-0 small text-truncate">{{ $attachment->file_name }}</p>
        ...
        <div id="chat">
          ...
          <div id="chatMessages" class="chat-messages mb-3" style="max-height: 400px; overflow-y: auto;">
            ...
            <div class="d-flex mb-3 justify-content-end">
              <div class="flex-grow-1 me-2" style="max-width: 70%;">
                <div class="bg-primary text-white rounded p-3">
                  <p class="mb-0 small">{{ $message->message }}</p>
                </div>
                <small class="text-muted d-block text-end mt-1">{{ $message->created_at->format('H:i') }}</small>
              </div>
              ...
              <div class="flex-grow-1 ms-2" style="max-width: 70%;">
                <div class="bg-light rounded p-3">
                  <p class="mb-0 small"><strong>{{ $message->user->name }}:</strong> {{ $message->message }}</p>
                </div>
    ...
    <div class="card shadow-sm border-0">
      <div class="card-body p-4">
        ...
        <div class="d-flex align-items-center gap-3 mb-3">
          <img src="{{ asset('assets/images/profile/user1.jpg') }}" class="rounded-circle" width="50" height="50">
          <div>
    ...
    <div class="card shadow-sm border-0 mt-4">
      <div class="card-body p-4">
        ...
        <div class="d-flex align-items-center gap-3">
          <img src="{{ asset('assets/images/profile/user-2.jpg') }}" class="rounded-circle" width="50" height="50">
          <div>
            <h6 class="mb-0 fw-medium">{{ $ticket->technician->name }}</h6>
            <p class="text-muted mb-0 small">{{ $ticket->technician->department }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="card-body p-4">
        <div class="d-flex align-items-center gap-2">
          ...
          <div>
            ...
            <p class="text-muted mb-0 small">Diselesaikan pada {{ $ticket->resolved_at->format('d M Y, H:i') }}</p>
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="d-flex gap-3 mb-3">
            <div class="flex-shrink-0">
              <div class="bg-primary rounded-circle p-1">
                <iconify-icon icon="solar:user-check-linear" class="text-white fs-5"></iconify-icon>
              </div>
            </div>
            <div>
          ...
          <div class="d-flex gap-3">
            <div class="flex-shrink-0">
              <div class="bg-warning rounded-circle p-1">
                <iconify-icon icon="solar:hourglass-circle-linear" class="text-white fs-5"></iconify-icon>
              </div>
            </div>
            <div>
