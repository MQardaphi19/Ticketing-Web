...
<head>
  ...
  <style>
    iconify-icon { display: inline-flex; align-items: center; justify-content: center; width: 1em; height: 1em; line-height: 1; }
    iconify-icon[class*="fs-1"] { font-size: 3.5rem; }
    iconify-icon[class*="fs-2"] { font-size: 3rem; }
    iconify-icon[class*="fs-3"] { font-size: 2.25rem; }
    iconify-icon[class*="fs-4"] { font-size: 1.5rem; }
    iconify-icon[class*="fs-5"] { font-size: 1.25rem; }
    iconify-icon[class*="fs-6"] { font-size: 1rem; }
    .btn-sm iconify-icon { font-size: 1rem; }
  </style>
  @stack('styles')
</head>
...
<body>
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
    ...
    <div class="modal-dialog">
        ...
        <nav class="sidebar-nav scroll-sidebar" data-simplebar="" style="flex-grow: 1; overflow-y: auto; overflow-x: hidden;">
          <ul id="sidebarnav" style="margin-bottom: 0; padding-bottom: 0;">
            ...
            <li class="sidebar-item mt-5 {{ request()->routeIs('users.index') ? 'selected' : '' }}">
              <a class="sidebar-link primary-hover-bg" aria-expanded="false" onclick="document.getElementById('logout-form').submit(); return false;" href="javascript:void(0);">
        ...
        <div class="sidebar-footer p-3 text-center" style="background: #f8f9fa; border-top: 1px solid #dee2e6; margin-top: 0;">
    ...
    <div class="body-wrapper" style="margin-left: 250px; padding-top: 70px; min-height: 100vh;">
      <header class="app-header" style="position: fixed; top: 0; left: 250px; right: 0; z-index: 999; background: #fff; height: 70px; border-bottom: 1px solid #dee2e6;">
        <nav class="navbar navbar-expand-lg navbar-light" style="margin: 0; padding: 0; height: 100%;">
          <div class="d-block d-lg-flex align-items-center justify-content-between px-4 w-100" style="height: 100%;">
            <div class="d-flex align-items-center gap-3">
              <a class="nav-link sidebartoggler nav-icon-hover d-block d-xl-none" id="headerCollapse" href="javascript:void(0)">
            ...
            <div class="d-flex align-items-center gap-3">
              <div class="dropdown">
                <a href="javascript:void(0)" class="nav-link position-relative" id="dropdownNotification" data-bs-toggle="dropdown">
                  <iconify-icon icon="mdi:bell" class="fs-6"></iconify-icon>
                  <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" id="notification-count">3</span>
                </a>
                <div class="dropdown-menu dropdown-menu-end dropdown-menu-animate-up py-0 overflow-hidden" aria-labelledby="dropdownNotification">
                  ...
                  <div class="p-3" style="max-height: 300px; overflow-y: auto;">
                    <div class="d-flex align-items-start gap-3 mb-3">
                      <div class="bg-primary-subtle rounded-circle p-2">
                        <iconify-icon icon="mdi:ticket" class="text-primary fs-5"></iconify-icon>
                      </div>
                      <div class="flex-grow-1">
                        <p class="mb-1 text-dark small">Tiket TIX-202602-001 telah diselesaikan</p>
                        <small class="text-muted">5 menit yang lalu</small>
                      </div>
                    </div>
                    <div class="d-flex align-items-start gap-3 mb-3">
                      <div class="bg-warning-subtle rounded-circle p-2">
                        <iconify-icon icon="mdi:chat" class="text-warning fs-5"></iconify-icon>
                      </div>
                      <div class="flex-grow-1">
                  ...
                  <a href="#" class="dropdown-item text-center py-2 border-top">Lihat Semua</a>
                </div>
              </div>

              <div class="dropdown">
                <a href="javascript:void(0)" class="nav-link d-flex align-items-center gap-2" id="dropdownUser" data-bs-toggle="dropdown">
                  <img src="{{ asset('assets/images/profile/user1.jpg') }}" alt="User" class="rounded-circle" width="35" height="35">
                  <div class="d-none d-lg-block">
                    <h6 class="mb-0 fw-semibold fs-6">{{ auth()->check() ? auth()->user()->name : 'Demo User' }}</h6>
                    <small class="text-muted">Super Admin</small>
                  </div>
                </a>
                <ul class="dropdown-menu dropdown-menu-end dropdown-menu-animate-up" aria-labelledby="dropdownUser">
                  <li><a class="dropdown-item" href="#"><iconify-icon icon="mdi:account" class="me-2"></iconify-icon>Profil Saya</a></li>
  ...
  <script>
    ...
    function replaceIconsWithFallback() {
      ...
      icons.forEach(function(icon) {
        ...
        let fallbackText = '';
        if (iconAttr.includes('pen') || iconAttr.includes('edit') || iconAttr.includes('pencil')) fallbackText = '✎';
        ...
        else if (iconAttr.includes('add') || iconAttr.includes('plus') || iconAttr.includes('circle')) fallbackText = '➕';
        else if (iconAttr.includes('folder')) fallbackText = '📁';
        else if (iconAttr.includes('book') || iconAttr.includes('bookmark')) fallbackText = '📖';
        else if (iconAttr.includes('chat') || iconAttr.includes('message') || iconAttr.includes('dots')) fallbackText = '💬';
        else if (iconAttr.includes('users') || iconAttr.includes('group') || iconAttr.includes('rounded')) fallbackText = '👥';
        ...
        else if (iconAttr.includes('close') || iconAttr.includes('x') || iconAttr.includes('ti-x')) fallbackText = '✕';
